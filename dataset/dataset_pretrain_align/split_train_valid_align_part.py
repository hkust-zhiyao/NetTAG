...
def run_one_ep(design, ep):
    ...
    if graph_data_ori.x.shape[1] != 4121:
        print(f"Graph data ori shape: {graph_data_ori.x.shape}")
        return None
    ...
    if graph_data_pos.x.shape[1] != 4121:
        print(f"Graph data pos shape: {graph_data_pos.x.shape}")
        return None
...
def save_dataset(design_lst, tag):
    ...
    for idx in range(k):
        start_idx = idx*ll//k
        end_idx = (idx+1)*ll//k
        rtl_dataset_part = rtl_dataset[start_idx:end_idx]
        netlist_ori_dataset_part = netlist_ori_dataset[start_idx:end_idx]
        netlist_pos_dataset_part = netlist_pos_dataset[start_idx:end_idx]
        layout_dataset_part = layout_dataset[start_idx:end_idx]

        with open (f"./data/{tag}_rtl_{idx}.pkl", 'wb') as f:
            pickle.dump(rtl_dataset_part, f)
        with open (f"./data/{tag}_netlist_ori_{idx}.pkl", 'wb') as f:
            pickle.dump(netlist_ori_dataset_part, f)
        with open (f"./data/{tag}_netlist_pos_{idx}.pkl", 'wb') as f:
            pickle.dump(netlist_pos_dataset_part, f)
        with open (f"./data/{tag}_layout_{idx}.pkl", 'wb') as f:
            pickle.dump(layout_dataset_part, f)
