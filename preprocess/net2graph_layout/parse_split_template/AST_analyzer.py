...
class AST_analyzer(object):
    ...
    def update_one_node(self, param):
        ...
        if node_dict[node].tpe not in [None, "Input"]:
            ...
            fan_in_nodes = nx.ancestors(subg, node)
            fan_in_nodes.add(node)
            input_subgraph = subg.subgraph(fan_in_nodes)
            
            fan_out_nodes = nx.descendants(subg, node)
            fan_out_nodes.add(node)
            output_subgraph = subg.subgraph(fan_out_nodes)
