...
class AST_analyzer(object):
    ...
    def update_ppa_info(self, design_name):
        ...
        if not os.path.exists(pt_info_dir):
            # print(f"Error: {pt_info_dir} not exists!")
            return
    ...
    def graph_split(self, save_dir, design_name):
        ...
        print('----- Updating Node Info -----')
        g_nx = nx.DiGraph(self.graph.graph)
        self.subgraph_annotation(g_nx)
        ...
        with open (f"{save_dir}/{design_name}/{design_name}_graph.pkl", 'wb') as f:
            pickle.dump(g_nx, f)
